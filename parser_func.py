...
def save_data(out_tweets, account):
    ...
    with open('%s_tweets.csv' % account, 'w', encoding='utf-8') as f:
        writer = csv.writer(f)
        writer.writerow(["user", "id", "created_at", "text", "location", "media_urls", "tweet_status"])
...
class TweetParser(object):
    ...
    def update_fetch(self, account):
        last_id = get_user_last_tweet(con)
    ...
    def first_fetch(self, account, last_id=None, tweet_mode='extended', limit=200):
        api = self.api
        count = 1
        video_urls = list()
        image_urls = list()
        for tweet in tweepy.Cursor(api.user_timeline, id=account, since_id=last_id, tweet_mode=tweet_mode).items(limit):
            ...
            tweet_status = 'Active'
            out_tweets = ([user_id, id_tweet, date_tweet, text_tweet, tweet_location, media_urls, tweet_status])
